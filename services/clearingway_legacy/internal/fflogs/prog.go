...
func (f *Fflogs) GetProgForReport(r string, rankingsToGet []*RankingToGet, char *ffxiv.Character) (*Fights, error) {
	...
	fmt.Fprintf(&query, "query{reportData{report(code: \"%s\") {fights {kill difficulty id encounterID lastPhaseAsAbsoluteIndex friendlyPlayers} masterData(translate: false) {actors(type: \"Player\") {id name server}}}}}",
		r)
