...
async def submit_form(request: FormRequest):
    ...
    if db_user:
        ...
        return {"detail": "There was an issue with the form submission, please try again"}
...
async def update_form(request: UpdateRequest):
    ...
    result = banforms_database.update_form(request.form_id, request.approval, request.approver_id)
    if result[0] == True:
