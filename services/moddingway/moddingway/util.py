...
def _split_chunks(message_content: str, from_index: int, max_chunk_length: int = 2000):
    ...
    # If remaining message is shorter than max chunk length, return the end
...
def chunk_message(message_content: str, max_chunk_length: int = 2000):
    from_index = 0
    # The 'to_index = 0' initialization is redundant as it's immediately overwritten.
    # The 'else: from_index = to_index' block is syntactically incorrect as it's not paired with an 'if'.
    # Assuming the intent was to simplify the from_index update to always be from_index = to_index.
